...
export default class FlightBoard {
  ...
  constructor() {
      ...
      this.flightTableHeader.innerHTML = `<th class="id">ID</th><th class="origin">Origin</th><th class="destination">Destination</th><th class="original-departed">Original Departed</th><th class="departed">Departed</th><th class="arrived">Arrived</th><th class="delay-reason">Delay Reason</th>`;
      this.flightTable.append(this.flightTableHeader);
  }

  // Add a row to table with flight details
  postFlight = (flight) => {
      let delayedStatus = flight.delayReason ? " | Delayed" : "";
      let flightRow = document.createElement('tr');
      let flightRowHTML = `
          <td>${flight.number}</td>
          <td>${flight.origin}</td>
          <td>${flight.destination}</td>
          <td>${flight.originalDepartureTime}</td>
          <td>${flight.departed}${delayedStatus}</td>
          <td class="scheduled">${flight.arrived}</td>
          <td>${flight.delayReason || 'N/A'}</td>
      `;
